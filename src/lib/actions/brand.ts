...
const DEFAULT_ELEMENT_STYLE: Record<BrandElementType, string> = {
  ...
  wordmark:
    "A wordmark logo: ONLY the company name rendered as bold custom lettering, in the style of top home-service branding agencies. Clean and highly legible at a distance (for truck wraps and yard signs), strong confident letterforms, isolated on a plain white background. Pure black-and-white line art — no color, black ink outlines only. Strict rule: the image must contain lettering ONLY — no mascot, no character, no animal, no person, no icon, no emblem, no badge or shield frame, and no illustration of any kind. If a reference photo shows a mascot next to a wordmark, ignore the mascot completely and copy only its lettering style.",
  background:
